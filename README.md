...
| `/api/v1/submit/{sheet_id}` | POST   | _(screenshot data)_ | Bearer authentication | Submits a new screenshot and populates the spreadsheet (`sheet_id`). Authentication is handled by the access key provided via the `Authorization: Bearer <access-key>` header.              |
